...
def list_users():
    """Lister les utilisateurs du système (Windows)."""
    try:
        # Utilisation de la commande 'net user' pour lister les utilisateurs sur Windows
        result = subprocess.run(["net", "user"], shell=False, text=True, capture_output=True)
        if result.returncode == 0:
            logging.info("Liste des utilisateurs :\n" + result.stdout)
        else:
            logging.error("Erreur lors de la récupération des utilisateurs :\n" + result.stderr)
    except Exception as e:
